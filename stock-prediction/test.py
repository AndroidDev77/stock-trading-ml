...
def plot_graph(model, data):
    ...
    plt.xlabel("Days: " + ticker + " " + date_now)
    plt.ylabel("Price")
    timestr = time.strftime("%Y%m%d-%H%M%S")
    plt.legend(["Actual Price", "Predicted Price"])
    plt.savefig("logs/"+ticker+"_"+date_now+ "_" + timestr+ ".png")
    plt.show()
...
def predict(model, data, classification=False):
    ...
    print(f"Predicted High: {predicted_price_high:.2f}$")
    print(f"Predicted Low: {predicted_price_low:.2f}$")
    return predicted_price

print("ticker: " +ticker)
print("lookup_step: " + str(LOOKUP_STEP))
print("model name: " + model_name)
# load the data
data = load_data(ticker, date_now, N_STEPS, lookup_step=LOOKUP_STEP, test_size=TEST_SIZE,
